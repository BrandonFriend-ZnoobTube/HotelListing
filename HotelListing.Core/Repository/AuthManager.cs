...
public class AuthManager : IAuthManager
{
	...
	{
		_logger.LogInformation("User is logging in {LoginDOEmail}", loginDO.Email);

		try
		{
			_user = await _userManager.FindByEmailAsync(loginDO.Email);
			var isValidUser = await _userManager.CheckPasswordAsync(_user, loginDO.Password);
		...
		{
			_logger.LogError(e, "Error has occurred in {LoginName}, with the user {LoginDOEmail}", nameof(Login), loginDO.Email);
			return new AuthResponseDO();
		}
	}
	...
	{
		_logger.LogInformation("Registration attempt for {UserDOEmail}", userDO.Email);
		...
		{
			_logger.LogError(e, "Registration attempt failed for {UserDOEmail}", userDO.Email);
			return null;
		}
	}
